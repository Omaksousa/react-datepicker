...
export default class Calendar extends React.Component {
  ...
  header = (date = this.state.date) => {
    ...
    return dayNames.concat(
      [0, 1, 2, 3, 4, 5, 6].map((offset) => {
        ...
        const weekDayClassName = this.props.weekDayClassName ? this.props.weekDayClassName(day) : undefined;

        return (
          <div key={offset} className={classnames("react-datepicker__day-name", weekDayClassName)}>
            {weekDayName}
          </div>
        );
      })
  ...
  renderPreviousButton = () => {
    ...
    if (this.props.showMonthYearPicker || this.props.showQuarterYearPicker || this.props.showYearPicker) {
    ...
    const isForYear = this.props.showMonthYearPicker || this.props.showQuarterYearPicker || this.props.showYearPicker;
    ...
    return (
      ...
      >
        {isForYear ? this.props.previousYearButtonLabel : this.props.previousMonthButtonLabel}
      </button>
    );
  };
  ...
  renderNextButton = () => {
    ...
    const { nextMonthAriaLabel = "Next Month", nextYearAriaLabel = "Next Year" } = this.props;
    ...
    return (
      ...
      >
        {isForYear ? this.props.nextYearButtonLabel : this.props.nextMonthButtonLabel}
      </button>
    );
  };
  ...
  renderCurrentMonth = (date = this.state.date) => {
    ...
    return (
      <div className={classes.join(" ")}>
        {formatDate(date, this.props.dateFormat, this.props.locale, this.props.calendar, "header")}
      </div>
    );
  };
      ...
      <div className="react-datepicker__footer">
        {isHijri ? (
          <button onClick={() => this.props.onCalendarTypeChange(nextType)}>
            {this.props.hijriButtonLabel ||
              (this.props.locale === "ar" ? "عرض التقويم الميلادي" : "Show Gregorian Calendar")}
          </button>
        ) : (
          <button onClick={() => this.props.onCalendarTypeChange(nextType)}>
            {this.props.gregorianButtonLabel ||
              (this.props.locale === "ar" ? "عرض التقويم الهجري" : "Show Hijri Calendar")}
          </button>
        )}
      </div>
    );
  ...
  renderDefaultHeader = ({ monthDate, i }) => (
    <div
      className={`react-datepicker__header ${
        this.props.showTimeSelect ? "react-datepicker__header--has-time-select" : ""
      }`}
    >
  ...
  renderCustomHeader = (headerArgs = {}) => {
    ...
    const prevYearButtonDisabled = yearDisabledBefore(this.state.date, this.props);

    const nextYearButtonDisabled = yearDisabledAfter(this.state.date, this.props);

    const showDayNames =
      !this.props.showMonthYearPicker && !this.props.showQuarterYearPicker && !this.props.showYearPicker;

    return (
      <div className="react-datepicker__header react-datepicker__header--custom" onFocus={this.props.onDropdownFocus}>
        ...
        {showDayNames && <div className="react-datepicker__day-names">{this.header(monthDate)}</div>}
      </div>
    );
  };
  ...
  renderHeader = (headerArgs) => {
    switch (true) {
      ...
      case this.props.showMonthYearPicker || this.props.showQuarterYearPicker || this.props.showYearPicker:
    ...
    if (this.props.showYearPicker) {
      return (
        <div className="react-datepicker__year--container">
          {this.renderHeader()}
          <Year onDayClick={this.handleDayClick} date={this.state.date} {...this.props} />
        </div>
      );
    }
  };
